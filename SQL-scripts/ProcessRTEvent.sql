...
-----------------------PROCESS RT EVENT with COMMUTER RAIL SCHEDULE ADHERENCE------------------------
...
INSERT INTO @unprocessed_events_all
	...
	WHERE
		...
		AND										--ADDED
			service_date = @current_service_date --ADDED 

--SELECT * FROM @unprocessed_events_all
...
--SELECT * FROM @unprocessed_events_file
	...
	WHERE file_time = (SELECT file_time FROM @unprocessed_files WHERE file_time_num = @file_time_num_current)
...
DECLARE @today_rt_cd_time_temp AS TABLE
	...
	INSERT INTO @today_rt_cd_time_temp
		...
		FROM
			@unprocessed_events_file uef --departure at d
...
--This table stores headway times at a stop between trips of all routes in real-time
	...
	FROM
		(
			...
			FROM
				@unprocessed_events_file y --y is the "current" trip
...
--SELECT * FROM @today_rt_departure_time_sec
...
DECLARE @today_rt_schedule_adherence_disaggregate AS TABLE
	...
	SELECT
			...
			,dd.stop_order_flag
FROM 
	dbo.today_rt_event da --need to have had an arrival already in order to have a schedule adherence disaggregate with a departure
JOIN
	@today_rt_departure_time_sec dd --real-time departure
ON
...
--SELECT * FROM @today_rt_schedule_adherence_disaggregate
...
--This table stores travel times in real-time
	...
	(
		...
		FROM
			@unprocessed_events_file uef --arrival at e
...
DECLARE @today_rt_abcde_time_temp AS TABLE
	...
	FROM
		(
			...
			FROM
				@today_rt_cde_time_temp y --y is the "current" trip
	...
	WHERE
		rn = 1
--SELECT * FROM @today_rt_abcde_time_temp
...
--insert into  today_rt tables from @today_rt tables
	...
	SELECT
			...
			,stop_order_flag
FROM 
	@today_rt_schedule_adherence_disaggregate 
...
--Update passenger weighted travel time vs. threshold tables
	...
	SELECT
		...
		,ttt.threshold_historical_average_travel_time_sec --added
		,ttt.threshold_scheduled_average_travel_time_sec --added
...
FROM @today_rt_bd_sr_same_time_temp htt
	...
	JOIN dbo.today_headway_time_sr_same_benchmark dtb
				ON
					(	
							htt.bd_route_id = dtb.route_id --added
						AND
							htt.bd_direction_id = dtb.direction_id
						AND
							htt.bd_stop_id = dtb.stop_id
...
UPDATE dbo.rt_event
	SET 
		event_processed_rt = 1
	FROM 
		dbo.rt_event rte, @unprocessed_events_file uef
	WHERE 
	...
	--DELETE
	--FROM @today_rt_departure_time_sec

	--DELETE
	--FROM @today_rt_arrival_time_sec
