USE [mbta_performance]
GO

--Last updated: 2016-06-20

/****** Object:  StoredProcedure [dbo].[PreProcessDaily]    Script Date: 6/20/2016 11:31:18 AM ******/
...
SET		@day_of_the_week = (SELECT DATENAME(dw,@service_date_process));

DECLARE @day_type_id VARCHAR(255);
SET @day_type_id = (SELECT day_type_id FROM dbo.config_day_type_dow WHERE day_of_the_week = @day_of_the_week);

IF @service_date_process NOT IN (SELECT service_date FROM dbo.service_date)
...
SET @day_type_id_exception = (SELECT day_type_id_exception FROM dbo.service_date WHERE service_date = @service_date_process)

IF @day_type_id_exception IS NOT NULL SET @day_type_id = @day_type_id_exception
...
WHERE
	...
	AND
		service_date > DATEADD(d,-30,@service_date_process) -- only comparing day being processed with previous 30 days
	AND
		CASE
...
WHERE
	...
	AND
		...
		OR
			r.route_type = 1
		OR
			r.route_type = 2 --added cr
		)
	AND
		(	
						(@day_of_the_week = 'Monday'	and monday = 1)
				OR
						(@day_of_the_week = 'Tuesday'	and tuesday = 1)
				OR
						(@day_of_the_week = 'Wednesday' and wednesday = 1)
				OR
						(@day_of_the_week = 'Thursday'	and thursday = 1)
				OR
						(@day_of_the_week = 'Friday'	and friday = 1)
				OR
						(@day_of_the_week = 'Saturday'	and saturday = 1)
				OR
						(@day_of_the_week = 'Sunday'	and sunday = 1)
		)
				AND
					c.start_date	<= @service_date_process
				AND	
					c.end_date	>= @service_date_process
UNION
...
WHERE
	...
	AND
		t.route_id = r.route_id
	AND
		cd.exception_type = 1 -- service added
	AND 
...
--table for daily stop times sec which helps calculate benchmarks, needs to be real table instead of temp for schedule adherence
...
CREATE NONCLUSTERED INDEX IX_daily_stop_times_sec_route_type
	ON dbo.daily_stop_times_sec(route_type);

CREATE NONCLUSTERED INDEX IX_daily_stop_times_sec_route_id
	ON dbo.daily_stop_times_sec(route_id);

CREATE NONCLUSTERED INDEX IX_daily_stop_times_sec_direction_id
	ON dbo.daily_stop_times_sec(direction_id);

CREATE NONCLUSTERED INDEX IX_daily_stop_times_sec_trip_id
	ON dbo.daily_stop_times_sec(trip_id);

CREATE NONCLUSTERED INDEX IX_daily_stop_times_sec_stop_sequence
	ON dbo.daily_stop_times_sec(stop_sequence);

CREATE NONCLUSTERED INDEX IX_daily_stop_times_sec_stop_id
	ON dbo.daily_stop_times_sec(stop_id);

CREATE NONCLUSTERED INDEX IX_daily_stop_times_sec_arrival_time_sec
	ON dbo.daily_stop_times_sec(arrival_time_sec);

CREATE NONCLUSTERED INDEX IX_daily_stop_times_sec_departure_time_sec
	ON dbo.daily_stop_times_sec(departure_time_sec);

CREATE NONCLUSTERED INDEX IX_daily_stop_times_sec_1
ON [dbo].[daily_stop_times_sec] ([route_type],[stop_order_flag])
...
WHERE 
	...
	AND 
		ss_min.trip_first_stop = st.stop_sequence
	AND 
		ss_min.trip_id = ti.trip_id
	AND 
		st.trip_id = ti.trip_id
...
WHERE 
	...
	AND 
		ss_max.trip_last_stop = st.stop_sequence 
	AND 
		ss_max.trip_id = ti.trip_id
	AND 
		st.trip_id = ti.trip_id
...
INSERT INTO dbo.daily_stop_times_sec
	...
	SELECT 
		...
		,wtt.trip_end_time
		,CASE
			WHEN sta.stop_id = wtt.trip_first_stop_id AND sta.stop_sequence = wtt.trip_first_stop_sequence THEN 1
			WHEN sta.stop_id = wtt.trip_last_stop_id AND sta.stop_sequence = wtt.trip_last_stop_sequence THEN 3
			ELSE 2 END AS stop_order_flag
...
--temp table for travel times which helps calculate benchmarks
	...
	WHERE
			ti.trip_id = sta.trip_id 
		AND
			sta.trip_id = stb.trip_id
		AND
			sta.stop_sequence < stb.stop_sequence
		;

CREATE NONCLUSTERED INDEX IX_stop_times_travel_time_sec_daily_route_type
	ON #daily_stop_times_travel_time_sec(route_type);

CREATE NONCLUSTERED INDEX IX_stop_times_travel_time_sec_daily_route_id 
	ON #daily_stop_times_travel_time_sec(route_id);

CREATE NONCLUSTERED INDEX IX_stop_times_travel_time_sec_daily_direction_id 
	ON #daily_stop_times_travel_time_sec(direction_id);

CREATE NONCLUSTERED INDEX IX_stop_times_travel_time_sec_daily_trip_id 
	ON #daily_stop_times_travel_time_sec(trip_id);

CREATE NONCLUSTERED INDEX IX_stop_times_travel_time_sec_daily_from_stop_sequence
	ON #daily_stop_times_travel_time_sec(from_stop_sequence);

CREATE NONCLUSTERED INDEX IX_stop_times_travel_time_sec_daily_from_stop_id
	ON #daily_stop_times_travel_time_sec(from_stop_id);

CREATE NONCLUSTERED INDEX IX_stop_times_travel_time_sec_daily_from_arrival_time_sec
	ON #daily_stop_times_travel_time_sec(from_arrival_time_sec);

CREATE NONCLUSTERED INDEX IX_stop_times_travel_time_sec_daily_from_departure_time_sec
	ON #daily_stop_times_travel_time_sec(from_departure_time_sec);

CREATE NONCLUSTERED INDEX IX_stop_times_travel_time_sec_daily_to_stop_sequence
	ON #daily_stop_times_travel_time_sec(to_stop_sequence);

CREATE NONCLUSTERED INDEX IX_stop_times_travel_time_sec_daily_to_stop_id
	ON #daily_stop_times_travel_time_sec(to_stop_id);

CREATE NONCLUSTERED INDEX IX_stop_times_travel_time_sec_daily_to_arrival_time_sec
	ON #daily_stop_times_travel_time_sec(to_arrival_time_sec);

CREATE NONCLUSTERED INDEX IX_stop_times_travel_time_sec_daily_to_departure_time_sec
	ON #daily_stop_times_travel_time_sec(to_departure_time_sec);

--Create daily_abcde_time table. This table stores the day being processed scheduled joined events (abcde_time)
...
(
	...
	FROM
		#daily_stop_times_travel_time_sec cde -- cde is travel from b/c to d
		...
		JOIN #daily_stop_times_travel_time_sec ab -- b is the departure of the last relevant train at the "From" stop. Relevant means that the train takes people from abcd ("From" stop) to e ("To" stop)
			ON (
					...
					AND
						cde.to_stop_id = ab.to_stop_id
					AND 
						cde.direction_id = ab.direction_id
					AND
						ab.route_type = cde.route_type
					AND
						cde.trip_id <> ab.trip_id
					AND 
						(cde.from_arrival_time_sec > ab.from_departure_time_sec OR (cde.from_arrival_time_sec = ab.from_departure_time_sec AND cde.trip_id > ab.trip_id)) --changed to address green line trains at same time issue
					AND
...
WHERE t.rn = 1
	...
	CREATE NONCLUSTERED INDEX IX_daily_abcde_time_scheduled_b_d_time_sec ON #daily_abcde_time_scheduled(abcd_stop_id,e_stop_id,abcde_direction_id,b_time_sec) INCLUDE (d_time_sec)
	
	CREATE NONCLUSTERED INDEX IX_daily_abcde_time_scheduled_d_e_time_sec ON #daily_abcde_time_scheduled(abcd_stop_id,e_stop_id,abcde_direction_id,d_time_sec) INCLUDE (e_time_sec)
...
CREATE NONCLUSTERED INDEX IX_daily_travel_time_benchmark_1
ON [dbo].[daily_travel_time_benchmark] ([from_stop_id],[to_stop_id],[route_id],[direction_id])
...
FROM
	...
	(
		...
		FROM
			(
				...
				FROM
					dbo.historical_travel_time_disaggregate att
					,dbo.config_time_slice
					,@historical_service_dates hsd
				WHERE 
						att.service_date = hsd.historical_service_date
					AND
						att.end_time_sec < time_slice_end_sec 
					AND
						att.end_time_sec >= time_slice_start_sec
			)t
		GROUP BY
			from_stop_id
			,to_stop_id
	...
	(
		...
		FROM
			(
				...
				FROM
					#daily_stop_times_travel_time_sec att
					,dbo.config_time_slice
				WHERE 
						to_arrival_time_sec < time_slice_end_sec 
					AND
						to_arrival_time_sec >= time_slice_start_sec
			)t
		GROUP BY
			from_stop_id
			,to_stop_id
	...
	ON --do we want to add route_type and route_id here? they could be null...
		(
				his.from_stop_id = sch.from_stop_id
			AND
				his.to_stop_id = sch.to_stop_id
			AND 
				his.route_id = sch.route_id --added in because of green line
			AND
				his.direction_id = sch.direction_id
			AND
				his.time_slice_id = sch.time_slice_id
		)
...
IF OBJECT_ID('dbo.daily_travel_time_threshold','U') IS NOT NULL
 ...
 CREATE TABLE dbo.daily_travel_time_threshold 
	...
	,threshold_historical_average_travel_time_sec	INT NULL --added 
	,threshold_scheduled_average_travel_time_sec	INT NULL --added
	)
...
(
	...
	,threshold_historical_average_travel_time_sec --added
	,threshold_scheduled_average_travel_time_sec --added
)
SELECT 
	att.service_date
	,att.from_stop_id
	...
	,th.threshold_id
	,CASE 
		WHEN th.min_max_equal='min'
			THEN MIN(att.historical_median_travel_time_sec * thc.multiply_by + thc.add_to)
		WHEN th.min_max_equal='max'
			THEN MAX(att.historical_median_travel_time_sec * thc.multiply_by + thc.add_to)
		WHEN th.min_max_equal='equal'
			THEN AVG(att.historical_median_travel_time_sec * thc.multiply_by + thc.add_to)
		ELSE 0
		END AS threshold_historical_median_travel_time_sec
	,CASE 
		WHEN th.min_max_equal='min'
			THEN MIN(att.scheduled_median_travel_time_sec * thc.multiply_by + thc.add_to)
		WHEN th.min_max_equal='max'
			THEN MAX(att.scheduled_median_travel_time_sec * thc.multiply_by + thc.add_to)
		WHEN th.min_max_equal='equal'
			THEN AVG(att.scheduled_median_travel_time_sec * thc.multiply_by + thc.add_to)
		ELSE 0
		END AS threshold_scheduled_median_travel_time_sec
	,CASE 
		WHEN th.min_max_equal='min'
			THEN MIN(att.historical_average_travel_time_sec * thc.multiply_by + thc.add_to)
		WHEN th.min_max_equal='max'
			THEN MAX(att.historical_average_travel_time_sec * thc.multiply_by + thc.add_to)
		WHEN th.min_max_equal='equal'
			THEN AVG(att.historical_average_travel_time_sec * thc.multiply_by + thc.add_to)
		ELSE 0
		END AS threshold_historical_average_travel_time_sec --added
	,CASE 
		WHEN th.min_max_equal='min'
			THEN MIN(att.scheduled_average_travel_time_sec * thc.multiply_by + thc.add_to)
		WHEN th.min_max_equal='max'
			THEN MAX(att.scheduled_average_travel_time_sec * thc.multiply_by + thc.add_to)
		WHEN th.min_max_equal='equal'
			THEN AVG(att.scheduled_average_travel_time_sec * thc.multiply_by + thc.add_to)
		ELSE 0
		END AS threshold_scheduled_average_travel_time_sec --added
				
	FROM 
	dbo.daily_travel_time_benchmark att
...
CREATE NONCLUSTERED INDEX IX_daily_headway_time_od_benchmark_route_type
ON [dbo].[daily_headway_time_od_benchmark] ([route_type])
...
CREATE NONCLUSTERED INDEX IX_daily_headway_time_od_benchmark_1
ON [dbo].[daily_headway_time_od_benchmark] ([stop_id],[to_stop_id],[direction_id])
	...
	(
		...
		FROM
			(
				...
				FROM
					dbo.historical_headway_time_od_disaggregate aht
					,dbo.config_time_slice
					,@historical_service_dates hsd
				WHERE 
						aht.service_date = hsd.historical_service_date
					AND
						aht.end_time_sec < time_slice_end_sec 
					AND
						aht.end_time_sec >= time_slice_start_sec
			)t
		GROUP BY
			stop_id
			,to_stop_id
	...
	(
		...
		FROM
			(
				...
				FROM
					#daily_abcde_time_scheduled att
					,dbo.config_time_slice
				WHERE 
						d_time_sec < time_slice_end_sec 
					AND
						d_time_sec >= time_slice_start_sec
			)t
		GROUP BY
			stop_id
			,to_stop_id
...
(
		...
		,threshold_historical_average_wait_time_sec INT NULL --added
		,threshold_scheduled_average_wait_time_sec INT NULL --added
	)
INSERT INTO dbo.daily_wait_time_od_threshold
	(
		...
		,threshold_historical_average_wait_time_sec --added
		,threshold_scheduled_average_wait_time_sec --added
	)
...
SELECT 
	...
	,CASE 
		WHEN th.min_max_equal='min'
			THEN MIN(aht.historical_median_headway_sec * thc.multiply_by + thc.add_to)
		WHEN th.min_max_equal='max'
			THEN MAX(aht.historical_median_headway_sec * thc.multiply_by + thc.add_to)
		WHEN th.min_max_equal='equal'
			THEN AVG(aht.historical_median_headway_sec * thc.multiply_by + thc.add_to)
		ELSE 0
	END AS threshold_historical_median_wait_time_sec
	,CASE 
		WHEN th.min_max_equal='min'
			THEN MIN(aht.scheduled_median_headway_sec * thc.multiply_by + thc.add_to)
		WHEN th.min_max_equal='max'
			THEN MAX(aht.scheduled_median_headway_sec * thc.multiply_by + thc.add_to)
		WHEN th.min_max_equal='equal'
			THEN AVG(aht.scheduled_median_headway_sec * thc.multiply_by + thc.add_to)
		ELSE 0
	END AS threshold_scheduled_median_wait_time_sec
	,CASE 
		WHEN th.min_max_equal='min'
			THEN MIN(aht.historical_average_headway_sec * thc.multiply_by + thc.add_to)
		WHEN th.min_max_equal='max'
			THEN MAX(aht.historical_average_headway_sec * thc.multiply_by + thc.add_to)
		WHEN th.min_max_equal='equal'
			THEN AVG(aht.historical_average_headway_sec * thc.multiply_by + thc.add_to)
		ELSE 0
	END AS threshold_historical_average_wait_time_sec --added
	,CASE 
		WHEN th.min_max_equal='min'
			THEN MIN(aht.scheduled_average_headway_sec * thc.multiply_by + thc.add_to)
		WHEN th.min_max_equal='max'
			THEN MAX(aht.scheduled_average_headway_sec * thc.multiply_by + thc.add_to)
		WHEN th.min_max_equal='equal'
			THEN AVG(aht.scheduled_average_headway_sec * thc.multiply_by + thc.add_to)
		ELSE 0
	END AS threshold_scheduled_average_wait_time_sec --added
	
	FROM 
	dbo.daily_headway_time_od_benchmark aht
	,dbo.config_threshold th
	,dbo.config_threshold_calculation thc
	,dbo.config_mode_threshold mt
	...
	(
		...
		FROM
			(
				...
				FROM
					dbo.historical_headway_time_sr_all_disaggregate aht
					,dbo.config_time_slice
					,@historical_service_dates hsd
				WHERE 
						aht.service_date = hsd.historical_service_date
				...
				( SELECT
					...
					FROM	(
						...
						WHERE
									...
									AND
										(y.departure_time_sec > x.departure_time_sec OR (y.departure_time_sec = x.departure_time_sec AND y.trip_id > x.trip_id)) --changed to address green line trains at same time issue
									AND 
...
(
		...
		,threshold_scheduled_average_headway_time_sec INT NULL --added
	)
...
SELECT 
	...
	,CASE 
		...
		WHEN th.min_max_equal='equal'
			THEN AVG(aht.scheduled_average_headway_sec * thc.multiply_by + thc.add_to)
		ELSE 0
	END AS threshold_scheduled_average_headway_time_sec --added
	
	FROM 
	dbo.daily_headway_time_sr_all_benchmark aht
	,dbo.config_threshold th
	,dbo.config_threshold_calculation thc
	,dbo.config_mode_threshold mt
	...
	AND
		mt.threshold_id = thc.threshold_id
	AND
		th.threshold_type = 'trip_headway_based'
	AND
		aht.route_type = mt.route_type
...
CREATE NONCLUSTERED INDEX IX_daily_headway_time_sr_same_benchmark_1
ON [dbo].[daily_headway_time_sr_same_benchmark] ([stop_id],[route_id],[direction_id])
INCLUDE ([time_slice_id],[scheduled_average_headway_sec])
...
SELECT
	@service_date_process
	,ISNULL(his.stop_id,sch.stop_id) AS stop_id
	,ISNULL(his.route_type,sch.route_type) AS route_type --added
	,ISNULL(his.route_id, sch.route_id) AS route_id --added
	,ISNULL(his.direction_id,sch.direction_id) AS direction_id
	,ISNULL(his.time_slice_id,sch.time_slice_id) AS time_slice_id
	,his.historical_average_headway_time_sec
	,his.historical_median_headway_time_sec
...
FROM
	...
	ON
		(
			his.route_id = sch.route_id --added
		AND
			his.stop_id = sch.stop_id
		AND
			his.direction_id = sch.direction_id
