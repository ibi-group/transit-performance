...
/****** Object:  StoredProcedure [dbo].[PostProcessDaily]    Script Date: 6/20/2016 11:30:19 AM ******/
...
--This procedure was updated on 12-13-2016 to add indices to historical tables and not include events with null event_time_sec in daily_events
...
--DECLARE @service_date_process DATE
--SET @service_date_process ='2016-11-16'
...
UPDATE dbo.event_rt_trip_archive
	...
	CREATE TABLE dbo.daily_event(
		record_id				INT PRIMARY KEY NOT NULL IDENTITY --previously used to take record_id from rt_event table
	...
	SELECT 
		...
		,event_time
		,event_time_sec
		,event_processed_rt --initialization scripts may not have these. need to check
		,event_processed_daily
	FROM dbo.rt_event ed, gtfs.routes r
	WHERE
...
IF OBJECT_ID('tempdb..##missed_events', 'U') IS NOT NULL
   ...
   DELETE FROM ##missed_events
    FROM ##missed_events me, dbo.daily_event de
    WHERE
...
IF OBJECT_ID('tempdb..##valid_trip_update_events', 'U') IS NOT NULL
    ...
    FROM dbo.event_rt_trip_archive tue --change to prod
	JOIN  ##missed_events me
		ON
		(
				tue.trip_id = me.trip_id
			AND
				tue.stop_id = me.stop_id
			AND
				tue.stop_sequence = me.stop_sequence
			AND
				tue.service_date = me.service_date
			AND
				tue.event_type = 'PRA'
			AND
				tue.event_time > 0
	...
	JOIN
            ...
            FROM
                dbo.event_rt_trip_archive --change to prod
			WHERE
				service_date = @service_date_process
            GROUP BY
                service_date
                ...
                ,event_type
            HAVING
                COUNT(vehicle_id) = 1) t
        ON
            (
                tue.service_date = t.service_date    
            AND
                tue.trip_id = t.trip_id
            AND
                tue.stop_sequence = t.stop_sequence
            AND
                tue.stop_id = t.stop_id
            )
...
SELECT
	...
	FROM
		(
			...
			FROM
				##valid_trip_update_events y --y is the "current" 
				...
				JOIN dbo.daily_event x --x is the most recent relevant "previous" 
					...
					ON (
						...
						AND
							y.trip_id = x.trip_id
						AND 
							y.event_time_sec > x.event_time_sec --make sure x DEP/ARR event is before y PRA event
						AND
							y.stop_sequence > x.stop_sequence --make sure x DEP/ARR event is before y PRA event
						AND
							y.event_type = 'PRA' 
						AND 
							x.suspect_record = 0
						AND 
							x.event_time_sec < 99000 --make sure x event is valid
						AND 
							y.file_time + 900 >= y.event_time --make sure the predicted event is less than 15 minutes from file time
					)
		)t
	WHERE
		rn = 1

--SELECT * FROM dbo.daily_event 
--WHERE route_id LIKE 'CR-%'
--AND stop_id = 'South Station'
--ORDER BY trip_id, stop_sequence, event_time_sec
	...
	FROM 
		...
		JOIN
		(
			SELECT service_date, route_id, vehicle_id, trip_id, count_event
			FROM
			(
				SELECT service_date, route_id, route_type, vehicle_id, trip_id, COUNT(*) AS count_event
				FROM dbo.daily_event ed
				GROUP BY service_date,route_id, route_type, vehicle_id, trip_id
			) t
				WHERE	
						t.count_event <= 2
					AND 
...
--SELECT * FROM dbo.daily_event
--WHERE route_type = 2
--ORDER BY trip_id, stop_sequence, event_time	
			...
			FROM
				...
				JOIN dbo.daily_event x --x is the most recent "previous" trip
					...
					ON (
						...
						--	y.route_id =x.route_id
						AND
							y.event_time_sec >= x.event_time_sec --Green Line at park can have two with exaclty the same time
						AND 
...
--SELECT * FROM ##daily_bd_sr_all_time
--WHERE bd_stop_id = '70061'
...
---first update for arrivals----------	
	--fills in the expected times for the missed events based on the actual times of the previous event and next event event
	...
	FROM daily_missed_stop_times_scheduled mst, 
		(
		SELECT mst.record_id, MIN (ed.stop_sequence) as min_after_stop_sequence, MIN(ed.event_time_sec) as min_after_event_time_arrival_sec
		FROM dbo.daily_event ed, daily_missed_stop_times_scheduled mst
		WHERE
				mst.trip_id = ed.trip_id
			AND
				ed.event_type = 'ARR'
		...
		WHERE mst.record_id = mb.record_id
	...
	FROM daily_missed_stop_times_scheduled mst, 
		(
		SELECT mst.record_id, MAX (ed.stop_sequence) as max_before_stop_sequence, MAX(ed.event_time_sec) as max_before_event_time_arrival_sec
		FROM dbo.daily_event ed, daily_missed_stop_times_scheduled mst
		WHERE
				mst.trip_id = ed.trip_id
			AND
		...
		WHERE mst.record_id = ma.record_id
...
--SELECT * FROM daily_missed_stop_times_scheduled
------then update for departures--------------------------------
	...
	FROM daily_missed_stop_times_scheduled mst, 
		(
		SELECT mst.record_id, MIN (ed.stop_sequence) as min_after_stop_sequence, MIN(ed.event_time_sec) as min_after_event_time_departure_sec
		FROM dbo.daily_event ed, daily_missed_stop_times_scheduled mst
		WHERE
				mst.trip_id = ed.trip_id
			AND
	...
	FROM daily_missed_stop_times_scheduled mst, 
		(
		SELECT mst.record_id, MAX (ed.stop_sequence) as max_before_stop_sequence, MAX(ed.event_time_sec) as max_before_event_time_departure_sec
		FROM dbo.daily_event ed, daily_missed_stop_times_scheduled mst
		WHERE
				mst.trip_id = ed.trip_id
			AND
	...
	UPDATE daily_missed_stop_times_scheduled
	SET
		max_before_departure_time_sec = st.departure_time_sec
	FROM daily_missed_stop_times_scheduled mst, 
		gtfs.stop_times st
	WHERE
			mst.trip_id = st.trip_id
...
--save the records we are going to delete from ##daily_abcde_time where the headway is missed at the from stop
	...
	SELECT 
			...
			,abcde.de_time_sec
			,abcde.bd_time_sec
	FROM ##daily_abcde_time abcde, daily_missed_stop_times_scheduled mst
	WHERE
...
-- DELETE where the headway is missed at the from stop------------------
	...
	FROM ##daily_bd_sr_same_time hsr, daily_missed_stop_times_scheduled mst
	WHERE
	...
	FROM ##daily_bd_sr_all_time hsr, daily_missed_stop_times_scheduled mst
	WHERE
			hsr.service_date = mst.service_date
		AND
			hsr.bd_stop_id = mst.stop_id
...
--put records that are DELETE to remove records where we know we missed the arrival at the to_stop (e) for abcde
INSERT INTO dbo.deleted_from_abcde_time
		(
			service_date
			,abcd_stop_id
	...
	,
	(
	SELECT mst.stop_id AS e_stop_id,cd.cd_stop_id,cd.d_time_sec FROM ##daily_cd_time cd, daily_missed_stop_times_scheduled mst
	WHERE 
...
--SELECT * FROM ##daily_abcde_time
--WHERE abcd_stop_id = '70079' AND e_stop_id = '70083'
--ORDER BY d_time_sec
	-----------------------------------------------------------------------------------------------------------
...
INSERT INTO dbo.deleted_from_abcde_time
	...
	SELECT 
			...
			,abcde.de_time_sec
			,abcde.bd_time_sec	
	FROM ##daily_abcde_time abcde, daily_event mst
	WHERE
			abcde.service_date = mst.service_date
		AND
			abcde.abcd_stop_id = mst.stop_id
	...
	DELETE FROM ##daily_bd_sr_same_time
	FROM ##daily_bd_sr_same_time hsr, daily_event mst
	WHERE
			hsr.service_date = mst.service_date
		AND
			hsr.bd_stop_id = mst.stop_id
	...
	DELETE FROM ##daily_bd_sr_all_time
	FROM ##daily_bd_sr_all_time hsr, daily_event mst
	WHERE
			hsr.service_date = mst.service_date
		AND
			hsr.bd_stop_id = mst.stop_id
		...
		AND
			mst.event_type = 'DEP'
---ADDED to remove headways between events with suspect records in the middle-------------------------------
...
--SELECT * FROM ##daily_abcde_time
	...
	(
		...
		,threshold_scheduled_average_travel_time_sec	INT NOT NULL --added
		...
		--,historical_threshold_numerator_trip			FLOAT  --added
	...
	(
		...
		--,historical_threshold_numerator_trip --added
	...
	SELECT
		...
		--	ELSE 0
		--	END AS scheduled_threshold_numerator_trip --added 
	...
	FROM ##daily_abcde_time abcde
			...
			LEFT JOIN dbo.config_passenger_arrival_rate par --changed to LEFT JOIN to account for o-ds that don't have pax rates
				ON (
			...
			JOIN dbo.daily_travel_time_threshold ttt
				ON
					(
						...
						AND
							abcde.cde_route_id = ttt.route_id --added for multiple routes visiting the same stops, greenl line
					)
...
--SELECT * FROM dbo.daily_travel_time_threshold_pax
...
----------------------------------------------------------CR travel times pax end ---------------------------------------------------------
	...
	(
		...
		,threshold_historical_average_wait_time_sec	INT  NULL --addded
		,threshold_scheduled_average_wait_time_sec	INT NOT NULL --added
		,denominator_pax							FLOAT  NULL
		,historical_threshold_numerator_pax			FLOAT  NULL
		,scheduled_threshold_numerator_pax			FLOAT  NULL
		--,denominator_trip							FLOAT NOT NULL --added
		--,historical_threshold_numerator_trip		FLOAT NOT NULL --added
		--,scheduled_threshold_numerator_trip			FLOAT NOT NULL --added
		)
	...
	SELECT
		...
		,wtt.threshold_historical_average_wait_time_sec --added
		,wtt.threshold_scheduled_average_wait_time_sec --added
		...
		--	ELSE 0
		--	END AS historical_threshold_numerator_trip --added
		--,CASE 
...
--save headway trip metrics
	...
	INSERT INTO dbo.daily_headway_time_threshold_trip 
		(
			...
			,threshold_scheduled_average_headway_time_sec --added
			,denominator_trip --added
			,scheduled_threshold_numerator_trip --added
		)
...
--save daily metrics for each route	
	...
	(
		...
		,metric_result		FLOAT 
		,metric_result_trip	FLOAT  NULL	--added
		,numerator_pax		FLOAT  NULL--added
		,denominator_pax	FLOAT  NULL --added
		,numerator_trip		FLOAT  NULL--added
...
INSERT INTO dbo.daily_metrics
	(
		...
		,metric_result_trip
		,numerator_pax --added
		,denominator_pax --added
		,numerator_trip	--added
		,denominator_trip --added
	)
	...
	SELECT
		...
		,NULL--,1 - SUM(scheduled_threshold_numerator_trip)/SUM(denominator_trip) AS metric_result_trip --added
...
SELECT
		...
		,NULL--,1-SUM(scheduled_threshold_numerator_trip)/SUM(denominator_trip) AS metric_result_trip --changed to scheduled
		,SUM(scheduled_threshold_numerator_pax) AS numerator_pax  --added
		,SUM(denominator_pax) AS denominator_pax --added
		,NULL--,SUM(scheduled_threshold_numerator_trip) AS numerator_trip --added
		,NULL--,SUM(denominator_trip) AS denominator_trip --added
...
SELECT
		...
		,ct.threshold_name
		,ct.threshold_type
		,NULL--,1-SUM(scheduled_threshold_numerator_pax)/SUM(denominator_pax) AS metric_result --changed to scheduled
		,1-SUM(scheduled_threshold_numerator_trip)/SUM(denominator_trip) AS metric_result_trip --changed to scheduled
		,NULL--,SUM(scheduled_threshold_numerator_pax) AS numerator_pax  --added
		,NULL--,SUM(denominator_pax) AS denominator_pax --added
		,SUM(scheduled_threshold_numerator_trip) AS numerator_trip --added
		,SUM(denominator_trip) AS denominator_trip --added
FROM
...
--this can be commented out and historical_metrics can be updated for 1,2,3 - as of 4-25-2016
--UPDATE dbo.daily_metrics
--  SET threshold_type = 'wait_time'
--  WHERE threshold_type = 'wait_time_headway_based'
...
CREATE TABLE dbo.daily_travel_time_disaggregate
	...
	SELECT
		...
		,htt.e_time_sec
		,htt.de_time_sec
		,dtb.scheduled_average_travel_time_sec --changed to scheduled average
...
SELECT
	...
	,htt.d_time_sec
	,htt.bd_time_sec
	,dtb.scheduled_average_headway_sec --changed to average
...
--SELECT * FROM daily_headway_time_sr_all_disaggregate
--WHERE stop_id = '70061'
...
(
	...
	--WITH (FORCESEEK, INDEX(IX_historical_event_service_date))
	WHERE
		service_date = @service_date_process 
) > 0 

DELETE FROM 
	dbo.historical_event
WHERE
	service_date = @service_date_process
...
(
	...
	--WITH (FORCESEEK, INDEX(IX_historical_travel_time_disaggregate_3))
	WHERE
		service_date = @service_date_process 
) > 0 

DELETE FROM 
	dbo.historical_travel_time_disaggregate
WHERE
	service_date = @service_date_process
...
(
	...
	--WITH (FORCESEEK, INDEX(IX_historical_headway_time_od_disaggregate_index_3))
	WHERE
		service_date = @service_date_process 
) > 0 

DELETE FROM 
	dbo.historical_headway_time_od_disaggregate
WHERE
	service_date = @service_date_process
...
(
	...
	--WITH (FORCESEEK, INDEX(IX_historical_headway_time_sr_all_disaggregate_index_3))
	WHERE
		service_date = @service_date_process 
) > 0 

DELETE FROM 
	dbo.historical_headway_time_sr_all_disaggregate
WHERE
	service_date = @service_date_process
...
(
	...
	--WITH (FORCESEEK, INDEX(IX_historical_headway_time_sr_same_disaggregate_index_3))
	WHERE
		service_date = @service_date_process 
) > 0 

DELETE FROM 
	dbo.historical_headway_time_sr_same_disaggregate
WHERE
	service_date = @service_date_process
...
(
	...
	--WITH (FORCESEEK, INDEX(IX_historical_dwell_time_disaggregate_service_id))
	WHERE
		service_date = @service_date_process 
) > 0 

DELETE FROM 
	dbo.historical_dwell_time_disaggregate
WHERE
	service_date = @service_date_process
...
(
	...
	--WITH (FORCESEEK, INDEX(IX_historical_schedule_adherence_disaggregate_service_date))
	WHERE
		service_date = @service_date_process 
) > 0 

DELETE FROM 
	dbo.historical_schedule_adherence_disaggregate
WHERE
	service_date = @service_date_process
...
(
	...
	--WITH (FORCESEEK, INDEX(IX_historical_schedule_adherence_threshold_pax_service_date))
	WHERE
		service_date = @service_date_process 
) > 0

DELETE FROM 
	dbo.historical_schedule_adherence_threshold_pax
WHERE
	service_date = @service_date_process
...
SELECT 
		...
		,denominator_pax
		,scheduled_threshold_numerator_pax
		,NULL-- denominator_trip
		,NULL--, scheduled_threshold_numerator_trip
...
(
	...
	--WITH (FORCESEEK, INDEX(IX_historical_travel_time_threshold_pax_service_date))
	WHERE
		service_date = @service_date_process 
) > 0 

DELETE FROM 
	dbo.historical_travel_time_threshold_pax
WHERE
	service_date = @service_date_process
...
SELECT  
		...
		,historical_threshold_numerator_pax
		,scheduled_threshold_numerator_pax
		,NULL--denominator_trip --added
		,NULL--historical_threshold_numerator_trip --added
		,NULL--scheduled_threshold_numerator_trip --added
...
(
	...
	--WITH (FORCESEEK, INDEX(IX_historical_wait_time_od_threshold_pax_service_date))
	WHERE
		service_date = @service_date_process 
) > 0 

DELETE FROM 
	dbo.historical_wait_time_od_threshold_pax
WHERE
	service_date = @service_date_process
...
(
	...
	--WITH (FORCESEEK, INDEX(IX_historical_headway_time_threshold_trip_service_date))
	WHERE
		service_date = @service_date_process 
) > 0

DELETE FROM 
	dbo.historical_headway_time_threshold_trip 
...
(
	...
	--WITH (FORCESEEK, INDEX(IX_historical_metrics_service_date))
	WHERE
		service_date = @service_date_process 
) > 0
...
SELECT 
		...
		,dm.threshold_type 
		,dm.metric_result
		,dm.metric_result_trip --added
		,dm.numerator_pax  --added
		,dm.denominator_pax --added
		,dm.numerator_trip --added
		,dm.denominator_trip --added
...
(
	...
	--WITH (FORCESEEK, INDEX(IX_historical_missed_stop_times_scheduled_service_date))
	WHERE
		service_date = @service_date_process 
) > 0

DELETE FROM 
	dbo.historical_missed_stop_times_scheduled
WHERE
	service_date = @service_date_process

--insert missed_stop times into historical table
