...
CREATE TABLE dbo.daily_journey_time_disaggregate
	...
	--,passengers_with_excess_journey_time	FLOAT			-- temporary, for validation only
...
SELECT
	...
	-- ,CASE	
		-- WHEN 
			-- (e_time_sec - b_time_sec) - (wt.expected_wait_time + ivt.expected_in_vehicle_time) <= 0							-- when max_excess_journey_time <= 0
				-- THEN 0																										-- then passengers_with_excess_journey_time = 0
		-- WHEN 
			-- (e_time_sec - b_time_sec) - (wt.expected_wait_time + ivt.expected_in_vehicle_time) > 0
			-- AND (e_time_sec - d_time_sec) - (wt.expected_wait_time + ivt.expected_in_vehicle_time) <= 0						-- when max_excess_journey_time > 0 and min_excess_journey_time <= 0
				-- THEN																											
					-- (
						-- (e_time_sec - b_time_sec) 
						-- - (wt.expected_wait_time + ivt.expected_in_vehicle_time)
					-- )
	...
	,CASE	
		...
		WHEN 
			...
			*
			...
			/ (par.passenger_arrival_rate * (d_time_sec - b_time_sec))															--total_passengers
			/ 60
		END																														AS excess_journey_time_per_passenger 
FROM	
