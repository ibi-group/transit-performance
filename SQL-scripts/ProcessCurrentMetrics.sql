...
/****** Object:  StoredProcedure [dbo].[ProcessRTEvent]    Script Date: 8/17/2015 2:56:11 PM ******/
...
CREATE TABLE #today_rt_current_metrics
	(
		...
		,metric_result_trip_last_hour	FLOAT  NULL --added
		,metric_result_trip_current_day	FLOAT  NULL --added
	)
...
INSERT INTO #current_metrics_day_til_now
	(
		...
		,metric_result_trip_current_day --added
	)
...
SELECT
		...
		,NULL--1 - SUM(tw2.scheduled_threshold_numerator_trip)/NULLIF(SUM(tw2.denominator_trip),0) AS metric_result_trip_current_day --added
FROM
	dbo.today_rt_wait_time_od_threshold_pax tw2
...
SELECT
		...
		,ct.threshold_name
		,ct.threshold_type
		,1-SUM(tt2.scheduled_threshold_numerator_pax)/NULLIF(SUM(tt2.denominator_pax),0) AS metric_result_current_day --changed to scheduled
		,NULL AS metric_result_trip_current_day--1 - SUM(tt2.scheduled_threshold_numerator_trip)/NULLIF(SUM(tt2.denominator_trip),0)  --changed to scheduled
		
FROM
	dbo.today_rt_travel_time_threshold_pax tt2 
	,dbo.config_threshold ct
WHERE
...
SELECT
		...
		,ct.threshold_name
		,ct.threshold_type
		,1-SUM(sa2.scheduled_threshold_numerator_pax)/NULLIF(SUM(sa2.denominator_pax),0) AS metric_result_current_day --changed to scheduled
		,NULL AS metric_result_trip_current_day--1 - SUM(sa2.scheduled_threshold_numerator_trip)/NULLIF(SUM(sa2.denominator_trip),0)  --changed to scheduled
	
FROM
	dbo.today_rt_schedule_adherence_threshold_pax sa2 
	,dbo.config_threshold ct
WHERE
...
SELECT
		...
		,ct.threshold_name
		,ct.threshold_type
		,NULL AS metric_result_current_day--,1-SUM(scheduled_threshold_numerator_pax)/SUM(denominator_pax) AS metric_result --changed to scheduled
		,1-SUM(scheduled_threshold_numerator_trip)/SUM(denominator_trip) AS metric_result_trip_current_day --changed to scheduled
FROM
	dbo.today_rt_headway_time_threshold_trip dtt 
	,dbo.config_threshold ct
WHERE
...
INSERT INTO #current_metrics_last_hour
	(
		...
		,metric_result_trip_last_hour --added
	)
...
SELECT
		...
		,NULL AS metric_result_trip_last_hour--1 - SUM(tw1.scheduled_threshold_numerator_trip)/NULLIF(SUM(tw1.denominator_trip),0)  --added
FROM
	dbo.today_rt_wait_time_od_threshold_pax tw1
	,dbo.config_threshold ct
WHERE
...
SELECT
		...
		,ct.threshold_name
		,ct.threshold_type
		,1-SUM(tt1.scheduled_threshold_numerator_pax)/NULLIF(SUM(tt1.denominator_pax),0)  AS metric_result_last_hour	 --changed to scheduled
		,NULL AS metric_result_trip_last_hour--1-SUM(tt1.scheduled_threshold_numerator_trip)/NULLIF(SUM(tt1.denominator_trip),0)  	--changed to scheduled
FROM
	dbo.today_rt_travel_time_threshold_pax tt1
	,dbo.config_threshold ct
WHERE
...
SELECT
		...
		,t.threshold_name
		,t.threshold_type
		,1-SUM(t.scheduled_threshold_numerator_pax)/NULLIF(SUM(t.denominator_pax),0)  AS metric_result_last_hour	 --changed to scheduled
		,NULL AS metric_result_trip_last_hour--1-SUM(t.scheduled_threshold_numerator_trip)/NULLIF(SUM(t.denominator_trip),0)  AS metric_result_trip_last_hour	--changed to scheduled
FROM
	(
	SELECT
		sa1.route_id
	...
	FROM
	dbo.today_rt_schedule_adherence_threshold_pax sa1
	,dbo.config_threshold ct
WHERE
...
SELECT
		...
		,ct.threshold_name
		,ct.threshold_type
		,NULL AS metric_result_last_hour--,1-SUM(scheduled_threshold_numerator_pax)/SUM(denominator_pax) AS metric_result --changed to scheduled
		,1-SUM(scheduled_threshold_numerator_trip)/SUM(denominator_trip) AS metric_result_trip_last_hour --changed to scheduled
FROM
	dbo.today_rt_headway_time_threshold_trip dtt 
	,dbo.config_threshold ct
WHERE
...
SELECT
	...
	,c1.metric_result_trip_last_hour --added
	,c2.metric_result_trip_current_day --added

FROM
	#current_metrics_last_hour c1
...
--SELECT * FROM #today_rt_current_metrics
