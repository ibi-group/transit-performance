...
--This stored procedure is called by the predictionmetrics API call.  It selects prediction metrics for a particular route, direction, stop for the requested time period

IF OBJECT_ID('getPredictionMetrics','P') IS NOT NULL
...
CREATE PROCEDURE dbo.getPredictionMetrics
	...
	,@stop_ids str_val_type READONLY
	,@from_time DATETIME
	,@to_time DATETIME
	...
	BEGIN --if a timespan is less than 6 hours and routes are only subway/light rail, then do the processing, if not return empty set
		...
		BEGIN
			...
			INSERT INTO @metricstemp
				...
				(
					...
					FROM	dbo.historical_wait_time_od_threshold_pax hwt
		...
		BEGIN
			...
			INSERT INTO @metricstemp
				...
				FROM	dbo.today_rt_wait_time_od_threshold_pax hwt
						,dbo.config_threshold ct
				WHERE
					ct.threshold_id = hwt.threshold_id
					AND (
					...
					AND DATEADD(s,hwt.end_time_sec,hwt.service_date) >= @from_time
					AND DATEADD(s,hwt.end_time_sec,hwt.service_date) <= @to_time
					AND route_id IN ('Red','Orange','Blue','Green-B','Green-C','Green-D','Green-E')
				GROUP BY
					ct.threshold_id
					,ct.threshold_name
				...
				FROM	dbo.today_rt_travel_time_threshold_pax hwt
						,dbo.config_threshold ct
				WHERE
					ct.threshold_id = hwt.threshold_id
					AND (
				...
				(
					...
					FROM	dbo.today_rt_schedule_adherence_threshold_pax hwt
							,dbo.config_threshold ct
					WHERE
						ct.threshold_id = hwt.threshold_id
						AND (
